...
const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const checkAuthState = async () => {
      try {
        const userJson = await AsyncStorage.getItem('@pep_mobile:user');
        const token = await AsyncStorage.getItem('@pep_mobile:token');
    ...
    try {
      ...
      if (username === 'usuario' && password === 'senha') {
        const mockUser: User = {
          id: '1',
          name: 'Usuário PEP',
          email: 'usuario@pep.com.br',
          role: 'medico',
        };

        // Armazene os dados do usuário e o token
        await AsyncStorage.setItem('@pep_mobile:user', JSON.stringify(mockUser));
        await AsyncStorage.setItem('@pep_mobile:token', 'mock-token-12345');
  ...
  const logout = async (): Promise<void> => {
    try {
      ...
      await AsyncStorage.removeItem('@pep_mobile:user');
      await AsyncStorage.removeItem('@pep_mobile:token');
